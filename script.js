...
function addGoogleReviewsButton() {
  ...
  if (menuLink) {
    actions.insertBefore(googleLink, menuLink);
  } else {
...
function addGoogleMap() {
  ...
  mapCard.innerHTML = `
    <iframe
      title="Map showing The Toll House Acton"
      src="https://www.google.com/maps?q=135%20High%20Street%2C%20Acton%2C%20London%20W3%206LY&output=embed"
      loading="lazy"
  ...
  style.textContent = `
    ...
    @media (max-width: 640px) {
      .google-map-card { border-radius: 22px; }
      .google-map-card iframe { height: 320px; }
    }
  `;
...
function createCookieConsent() {
  ...
  style.textContent = `
    .cookie-consent-banner{position:fixed;left:24px;right:24px;bottom:24px;z-index:9999;max-width:980px;margin:0 auto;padding:22px;border:1px solid rgba(85,148,177,.22);border-radius:26px;background:#fff;box-shadow:0 24px 80px rgba(31,19,13,.22);color:#2a211b;display:grid;grid-template-columns:1fr auto;gap:20px;align-items:center}
    .cookie-consent-banner[hidden],.cookie-settings-panel[hidden]{display:none!important}
    .cookie-consent-banner h2,.cookie-settings-panel h2{margin:0 0 8px;font-family:Inter,system-ui,sans-serif;font-size:1.1rem;color:#5b3826}
    .cookie-consent-banner p,.cookie-settings-panel p{margin:0;color:#5f524d;line-height:1.55;font-size:.96rem}
    .cookie-actions{display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-end}
    .cookie-btn{min-height:42px;padding:0 15px;border:1px solid rgba(85,148,177,.26);border-radius:999px;background:#eef6f9;color:#5b3826;font:inherit;font-weight:800;cursor:pointer}
    .cookie-btn.primary{background:#5594b1;color:#fff;border-color:#5594b1}
    .cookie-btn.dark{background:#5b3826;color:#fff;border-color:#5b3826}
    .cookie-settings-panel{position:fixed;inset:0;z-index:10000;display:grid;place-items:center;padding:20px;background:rgba(31,19,13,.58)}
    .cookie-settings-card{width:min(720px,100%);max-height:92vh;overflow:auto;padding:24px;border-radius:28px;background:#fff;box-shadow:0 24px 90px rgba(31,19,13,.34)}
    .cookie-option{display:flex;justify-content:space-between;gap:18px;padding:18px 0;border-top:1px solid rgba(85,148,177,.18)}
    .cookie-option strong{display:block;margin-bottom:6px;color:#5b3826}.cookie-option small{display:block;color:#5f524d;line-height:1.5}
    .cookie-toggle{width:54px;height:30px;flex:0 0 54px;appearance:none;border-radius:999px;background:#d8c8bd;position:relative;cursor:pointer;border:0}
    .cookie-toggle::after{content:'';position:absolute;width:24px;height:24px;top:3px;left:3px;border-radius:50%;background:#fff;transition:transform .18s ease;box-shadow:0 2px 8px rgba(31,19,13,.18)}
    .cookie-toggle:checked{background:#5594b1}.cookie-toggle:checked::after{transform:translateX(24px)}.cookie-toggle:disabled{opacity:.7;cursor:not-allowed}
    .cookie-floating-settings{position:fixed;right:16px;bottom:16px;z-index:9998;min-height:36px;padding:0 11px;border:1px solid rgba(85,148,177,.22);border-radius:999px;background:#fff;color:#5b3826;font:inherit;font-size:.82rem;font-weight:800;box-shadow:0 14px 40px rgba(31,19,13,.16);cursor:pointer}
    @media(max-width:720px){.cookie-consent-banner{left:12px;right:12px;bottom:12px;grid-template-columns:1fr;padding:18px}.cookie-actions{justify-content:stretch}.cookie-btn{width:100%}.cookie-settings-card{padding:20px;border-radius:22px}.cookie-floating-settings{right:12px;bottom:12px;min-height:34px;padding:0 10px;font-size:.78rem}}
  `;
  document.head.appendChild(style);
  ...
  function saveAndClose(settings) { saveCookieConsent(settings); banner.hidden = true; closeSettings(); }
